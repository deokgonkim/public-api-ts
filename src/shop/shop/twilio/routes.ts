...
router.post("/whatsapp", async (req, res) => {
  ...
  if (whatsappMessage.Body.startsWith("My Order")) {
    ...
    const [customerId, orderId] =
      whatsappMessage.Body.match(regexp)[1]?.split(",");
    ...
    if (customer) {
      await Customers.updateCustomer(customerId, {
        whatsappLink: `https://wa.me/${whatsappMessage.From.replace(
          "whatsapp:",
          ""
        )}`,
      });
  ...
  const mentioningBoss =
    whatsappMessage.Body?.startsWith("사장님") ||
    whatsappMessage.Body?.startsWith("Boss");
